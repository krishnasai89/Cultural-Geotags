...
export default function ArtifactCard({ item }) {
  ...
  useEffect(() => {
    ...
    gsap.fromTo(
      ...
      },
      {
        ...
        ease: "power2.out",
        scrollTrigger: {
          trigger: el, // The element that starts the animation
    ...
    // Clean up animation triggers when component unmounts
    return () => {
  ...
  return (
    <div
      ref={cardRef}
      className={`group relative overflow-hidden rounded-2xl border border-white/10 bg-slate-900 opacity-0 transition-all duration-500 hover:border-emerald-500/50 hover:shadow-[0_0_30px_rgba(16,185,129,0.15)] ${sizeClasses}`}
    >
      ...
      {/* Spatial Vignette Gradient Overlay */}
