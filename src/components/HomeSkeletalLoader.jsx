// src/components/HomeSkeletalLoader.jsx
"use client";

export default function HomeSkeletalLoader() {
  // Generates 6 placeholder wireframes to fill the asymmetric grid initial viewport canvas
  ...
  return (
    <div className="relative z-10 max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 auto-rows-[280px] animate-pulse">
      {placeholders.map((_, idx) => {
        // Simulates random size variance to match your original tall/wide asymmetric profiles
        let dimensionClasses = "col-span-1 row-span-1";
        if (idx === 1) dimensionClasses = "col-span-1 row-span-2"; // tall mimicry
        if (idx === 3) dimensionClasses = "md:col-span-2 row-span-1"; // wide mimicry
